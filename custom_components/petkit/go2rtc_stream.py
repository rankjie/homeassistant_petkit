"""Helpers for exposing the mirror relay through HA-managed go2rtc."""
...
class PetkitGo2RTCStreamManager:
    """Manage PetKit relay streams in HA-managed go2rtc."""
    ...
    def internal_webrtc_source(self, device_id: str) -> str | None:
        """Return the internal WHEP mirror source URL for go2rtc."""
    ...
    async def async_ensure_stream(self, device_id: str) -> str | None:
        ...
        if source is None:
            LOGGER.debug(
                "PetKit relay stream %s unavailable: internal HA HTTP endpoint unsupported",
                device_id,
            )
            return None
        ...
        async with lock:
            ...
            LOGGER.warning(
                "Failed to register PetKit relay stream %s in HA-managed go2rtc (%s)",
                device_id,
                ", ".join(statuses),
            )
            return None
